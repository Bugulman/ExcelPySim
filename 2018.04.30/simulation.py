...
def drawPvtCurve():
    ...
    df_PVTo = (
        sht.range((iRowBegin, 2), (iRowEnd, 5)).options(pd.DataFrame, index=False).value
    )
    ...
    iRowEnd = getKeyWordEndRow(iRowBegin)
    df_PVTg = (
        sht.range((iRowBegin, 2), (iRowEnd, 4)).options(pd.DataFrame, index=False).value
    )
...
def drawWells():
    ...
    while sht.range(index + str(iRow)).value != "END":
        ...
        if kw == u"General Info":
            ...
            df_prod = (
                sht.range((iRow, 2), (iEndProdRow, 7))
...
def readScheduleSection():
    ...
    while sht.range(index + str(iRow)).value != "END":
        ...
        if kw == u"General Info":
            ...
            iEndProdRow = getKeyWordEndRow(iRow)
            prod_date = (
                sht.range((iRow + 1, 2), (iEndProdRow, 2)).options(numbers=str).value
            )
            df_prod = (
                sht.range((iRow, 3), (iEndProdRow, 7))
                .options(pd.DataFrame, index=False)
                .value
            )
            iRow = iEndProdRow
    ...
    for well in wells.values():
        ...
        wellentry.create_dataset(
            "date", (len(well.prod_date),), data=well.prod_date, dtype=dt
        )
...
##
# with h5py.File('simModel.hdf5') as var:
#     print(var.keys())
#     staticG = var['StaticGroup']
#     print(staticG.keys())
...
def dumpPvtSection(f, fModel):
    ...
    for n in datasetNames:
        if n == "PvtGroup":
            ...
            mx_pvtw = df_pvtw.values
            mx_pvto = df_merged.values
...
def dumpInitSection(f, fModel):
    ...
    f.write("PCGRAVITY\n")
    f.write(
        "DEPTH  {}  DWOC  {}  DGOC  {}  PCWOR  0.  PCGOC  0.\n".format(
            fRefDepth, fwoc, fgoc
        )
    )
    ...
    for n in datasetNames:
        if n == "DynamicGroup":
            ...
            for wellG in fieldG.keys():
                ...
                df_prod[u"wopr"] = (
                    df_prod[u"Monthly Oil Production"] / df_prod[u"Production Days"]
                )
            ...
            for wellG in fieldG.keys():
                ...
                f.write(
                    "WELL  {}  '{}'  PRODUCER  VERTICAL  {}  {}\n".format(
                        wellId, wellName, headI, headJ
                    )
                )
            ...
            for index, prodItem in df_prodByDate.iterrows():
                ...
                f.write(
                    "    WELL {}  '{}'  BHP   14.7   MAXOIL  {:.3f}\n".format(
                        wellId, wellName, wopr
                    )
                )
